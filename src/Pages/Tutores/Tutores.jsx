...
const Tutores = () => {
  ...
  const handleDelete = (id) => {
    if (window.confirm("¿Estás seguro de que deseas eliminar este tutor?")) {
      fetch(
        `${import.meta.env.VITE_API_URL}/api/admin/DeleteTutores?id_persona=${id}`,
        {
          method: "DELETE",
          credentials: "include",
        },
        ...
        .then((response) => {
          if (response.ok) {
            ...
            setFilteredData((prevData) =>
              prevData.filter((tutor) => tutor.id !== id),
            );
  ...
  return (
    <div
      className={`tutores-container ${navbarCollapsed ? "navbar-collapsed" : ""}`}
    >
      <Navbar />
