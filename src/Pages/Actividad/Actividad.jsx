...
const Actividades = () => {
  ...
  const handleSaveActividad = () => {
    ...
    fetch(`${import.meta.env.VITE_API_URL}/api/admin/ActualizarActividad`, {
      ...
      .then((data) => {
        ...
        setData((prev) =>
          prev.map((a) =>
            a.id_actividad === actividadActualizada.id_actividad
      ...
      .catch((error) => {
        ...
        alert(
          "Hubo un problema al actualizar. Revisa la consola para más detalles.",
        );
      });
  };
  ...
  const handleBonusSubmit = async (event) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/api/admin/BonificarPuntos`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
    ...
    if (column === "Acciones") {
      return (
        <div className="action-buttons">
          ...
          <button
            className="action-button"
            onClick={() => handleTakeAttendance(row)}
          >
            <img
  ...
  return (
    <div
      className={`actividades-container ${navbarCollapsed ? "navbar-collapsed" : ""}`}
    >
      {renderNavbar()}
        ...
        {showBonusForm && bonusActividad && (
          <div className="bonus-container">
            <div className="bonus-header">
              <div>
                ...
                <p>
                  ...
                  <strong>{bonusActividad.nombre_actividad}</strong>
                </p>
              </div>
              <Button
                text="Cerrar"
            ...
            <form className="bonus-form" onSubmit={handleBonusSubmit}>
              <div className="input-field">
                <label htmlFor="bonus-estudiante-search">
                  Buscar estudiante
                </label>
                <TextField
              ...
              {bonusFeedback.error && (
                <div className="bonus-feedback error">
                  {bonusFeedback.error}
                </div>
              )}
